...
import {
  ...
  ArrowRight, Clock, FileText, Plus, Activity
} from "lucide-react";
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <motion.button key={p.id} onClick={() => { navigate("/compliance"); onClose(); }}
                className="w-full p-4 rounded-2xl border border-gray-100 hover:border-primary/30 text-left group transition-all"
...
const Dashboard = () => {
  ...
  const refreshAlerts = useCallback((uid: string) => {
    ...
    // Small delay so plots are loaded before running checks
    setTimeout(() => {
  ...
  const stats = [
    { key: "total" as SheetType, label: "Total Plots", value: total, color: "#007AFF", bg: "rgba(0,122,255,0.1)", icon: Map },
    { key: "compliant" as SheetType, label: "Compliant", value: compliant, color: "#34C759", bg: "rgba(52,199,89,0.1)", icon: CheckCircle2 },
    { key: "pending" as SheetType, label: "Pending Review", value: pending, color: "#FF9F0A", bg: "rgba(255,159,10,0.1)", icon: Clock },
    { key: "defaulting" as SheetType, label: "Defaulting", value: defaulting, color: "#FF3B30", bg: "rgba(255,59,48,0.1)", icon: AlertTriangle },
  ];

  const quickActions = [
    { icon: Plus, label: "Add Plot", desc: "Register new allotment", path: "/plots", color: "#007AFF" },
    { icon: ClipboardCheck, label: "Check Compliance", desc: "Review all plots", path: "/compliance", color: "#34C759" },
    { icon: Bot, label: "Ask AI", desc: "Compliance questions", path: "/assistant", color: "#5E5CE6" },
    { icon: BarChart3, label: "View Reports", desc: "Analytics & summaries", path: "/reports", color: "#FF9F0A" },
  ];
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none -z-10">
          <Orb x="72%" y="-5%" size={300} color="rgba(0,122,255,0.07)" delay={0} />
          <Orb x="-5%" y="35%" size={260} color="rgba(52,199,89,0.06)" delay={2} />
          <Orb x="80%" y="60%" size={220} color="rgba(255,59,48,0.05)" delay={1.5} />
        </div>
        ...
        {/* Animated Donut-to-Pie Chart */}
        <motion.div className="mt-5 rounded-2xl p-6 widget-card cursor-pointer group flex flex-col items-center"
          initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.15 }}
          onClick={() => setChartMode(chartMode === "donut" ? "pie" : "donut")}>

          <div className="flex items-center justify-between w-full mb-2">
            <h2 className="text-sm font-bold tracking-tight" style={{ color: "#1D1D1F" }}>Compliance Distribution</h2>
            <div className="px-2 py-1 rounded-full text-[10px] font-bold bg-black/5 text-gray-500 uppercase tracking-widest">
              Click to {chartMode === "donut" ? "expand" : "collapse"}
            </div>
          </div>

          <div className="relative flex items-center justify-center py-4 w-full">
            <svg width={220} height={220} viewBox="0 0 240 240" className="transform -rotate-90 filter drop-shadow-sm group-hover:scale-[1.02] transition-transform duration-500">
              {/* Background track */}
              <motion.circle cx={120} cy={120} r={60} fill="transparent"
                stroke="rgba(0,0,0,0.03)"
                animate={{ strokeWidth: chartMode === "pie" ? 120 : 28 }}
                transition={{ duration: 0.7, type: "spring", bounce: 0.3 }} />

              {/* Compliant segment */}
              <motion.circle cx={120} cy={120} r={60} fill="transparent" stroke="#34C759"
                animate={{ strokeWidth: chartMode === "pie" ? 120 : 28 }}
                strokeDasharray={`${(compliant / total || 0) * 377} 377`} strokeDashoffset={0}
                transition={{ duration: 0.7, type: "spring", bounce: 0.3 }} />

              {/* Pending segment */}
              <motion.circle cx={120} cy={120} r={60} fill="transparent" stroke="#FF9F0A"
                animate={{ strokeWidth: chartMode === "pie" ? 120 : 28 }}
                strokeDasharray={`${(pending / total || 0) * 377} 377`}
                strokeDashoffset={-((compliant / total || 0) * 377)}
                transition={{ duration: 0.7, type: "spring", bounce: 0.3 }} />

              {/* Defaulting segment */}
              <motion.circle cx={120} cy={120} r={60} fill="transparent" stroke="#FF3B30"
                animate={{ strokeWidth: chartMode === "pie" ? 120 : 28 }}
                strokeDasharray={`${(defaulting / total || 0) * 377} 377`}
                strokeDashoffset={-(((compliant + pending) / total || 0) * 377)}
                transition={{ duration: 0.7, type: "spring", bounce: 0.3 }} />
            </svg>

            {/* Center text fades out when it becomes a solid pie */}
            <AnimatePresence>
              {chartMode === "donut" && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }}
                  className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-4xl font-black tracking-tighter" style={{ color: "#1D1D1F" }}>
                    {complianceRate}%
                  </span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {stats.map((s, i) => (
            <motion.button key={s.key} onClick={() => setOpenSheet(s.key)} className="text-left focus:outline-none group"
              initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
              transition={{ delay: i * 0.07, type: "spring", damping: 20, stiffness: 260 }}>
              <div className="widget-card p-5 relative overflow-hidden h-full transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-1">
                <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full opacity-[0.09] group-hover:opacity-[0.18] transition-opacity pointer-events-none"
                  style={{ background: s.color }} />
                <div className="flex items-start justify-between relative">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: "#8A8A8E" }}>{s.label}</p>
                    <motion.p className="text-3xl font-bold mt-1.5 tabular-nums" style={{ color: "#1D1D1F" }}
                      initial={{ scale: 0.5 }} animate={{ scale: 1 }}
                      transition={{ delay: i * 0.07 + 0.2, type: "spring", damping: 14 }}>
                      {s.value}
                    </motion.p>
                    <p className="text-[10px] mt-1 font-semibold flex items-center gap-0.5" style={{ color: s.color }}>
                      View all <ArrowRight className="w-3 h-3" />
                    </p>
                  </div>
                  <motion.div className="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
                    style={{ background: s.bg }}
                    whileHover={{ scale: 1.12, rotate: 8 }} transition={{ type: "spring", damping: 10, stiffness: 280 }}>
                    <s.icon className="w-5 h-5" style={{ color: s.color }} />
                  </motion.div>
                </div>
                <div className="absolute bottom-0 left-0 right-0 h-0.5 rounded-b-[20px] opacity-0 group-hover:opacity-100 transition-opacity duration-300"
        ...
        <motion.div className="grid lg:grid-cols-3 gap-5 mb-8"
          ...
          <div>
            <h2 className="text-base font-bold mb-3 tracking-tight flex items-center gap-2" style={{ color: "#1D1D1F" }}>
              <Activity className="w-4 h-4" style={{ color: "#007AFF" }} /> Quick Actions
            </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold tracking-tight flex items-center gap-2" style={{ color: "#1D1D1F" }}>
                <Map className="w-4 h-4" style={{ color: "#007AFF" }} /> Recent Plots
              </h2>
              <button onClick={() => navigate("/plots")} className="text-xs font-semibold flex items-center gap-1" style={{ color: "#007AFF" }}>
                View all <ArrowRight className="w-3 h-3" />
              </button>
            </div>
            <div className="space-y-3">
                ...
                return (
                  <motion.button key={p.id} onClick={() => navigate("/compliance")}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <ArrowRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" style={{ color: "#007AFF" }} />
                    </div>
                  </motion.button>
                );
              })}
          ...
          <div>
            <h2 className="text-base font-bold tracking-tight flex items-center gap-2 mb-4" style={{ color: "#1D1D1F" }}>
              <Clock className="w-4 h-4" style={{ color: "#007AFF" }} /> Activity Log
            </h2>
